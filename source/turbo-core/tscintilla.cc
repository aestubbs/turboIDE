...
TScintilla::TScintilla()
{
    ...
    // Move lines up/down on Alt+Shift only. Ctrl+Shift+Arrows is reserved below
    // for column (rectangular) selection, the Sublime-style binding.
    ...
    // Column / rectangular selection: Ctrl+Shift+Arrows/Home/End/PgUp/PgDn.
    Msg(SCI_ASSIGNCMDKEY, SCK_UP    | ((SCMOD_SHIFT | SCMOD_CTRL) << 16), SCI_LINEUPRECTEXTEND);
    Msg(SCI_ASSIGNCMDKEY, SCK_DOWN  | ((SCMOD_SHIFT | SCMOD_CTRL) << 16), SCI_LINEDOWNRECTEXTEND);
    Msg(SCI_ASSIGNCMDKEY, SCK_LEFT  | ((SCMOD_SHIFT | SCMOD_CTRL) << 16), SCI_CHARLEFTRECTEXTEND);
    Msg(SCI_ASSIGNCMDKEY, SCK_RIGHT | ((SCMOD_SHIFT | SCMOD_CTRL) << 16), SCI_CHARRIGHTRECTEXTEND);
    Msg(SCI_ASSIGNCMDKEY, SCK_HOME  | ((SCMOD_SHIFT | SCMOD_CTRL) << 16), SCI_VCHOMERECTEXTEND);
    Msg(SCI_ASSIGNCMDKEY, SCK_END   | ((SCMOD_SHIFT | SCMOD_CTRL) << 16), SCI_LINEENDRECTEXTEND);
    Msg(SCI_ASSIGNCMDKEY, SCK_PRIOR | ((SCMOD_SHIFT | SCMOD_CTRL) << 16), SCI_PAGEUPRECTEXTEND);
    Msg(SCI_ASSIGNCMDKEY, SCK_NEXT  | ((SCMOD_SHIFT | SCMOD_CTRL) << 16), SCI_PAGEDOWNRECTEXTEND);
    ...
    // 'Delete line' on Ctrl+K. Ctrl+Shift+L is now "Split Selection into Lines"
    // (handled as an app command in editwindow.cc).
